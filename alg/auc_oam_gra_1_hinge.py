...
import timeit
from common.utils import get_idx, get_res_idx
import sys
import os
...
def auc_oam_gra_1_hinge(x_tr, y_tr, x_te, y_te, options):
    ...
    buf_pos = np.zeros(dim) # initiate buffer
    buf_neg = np.zeros(dim)
    n_pos = 0
    n_neg = 0
...
def auc_oam_gra_1_hinge_(x_tr, y_tr, x_te, y_te, options):
    n_tr = options['n_tr']
    dim = options['dim']
    n_pass = options['n_pass']
    start = timeit.default_timer()
...
if __name__ == '__main__':
    from common.utils import data_processing
